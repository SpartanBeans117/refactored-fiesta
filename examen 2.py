...
area_texto = scrolledtext.ScrolledText(root, width=40, height=10, wrap=tk.WORD, fg='black')
...
class FileManager:
    ...
    def BuscaryRemplazar(self, content):
        try:
            with open(self.file_path, 'w+') as file:
                file.write(content)
                return "Archivo remplazado exitosamente."
        except IOError:
            return "Error al remplazar el archivo."
...
def ByRemplazar():
    ...
    if file_path:
        content = area_texto.get(1.0, tk.END)
        file_manager = FileManager(file_path)
        message = file_manager.BuscaryRemplazar(content)
        messagebox.showinfo("Buscar y Remplazar", message)
...
def cambiarColor():
    ...
    color_actual = area_texto.cget("fg")  
    nuevo_color = colores[(colores.index(color_actual) + 1) % len(colores)]
    area_texto.config(fg=nuevo_color)     
